...
for i in numpy.arange(0, len(timescale)):
    ...
    current = (dataset.filter(ee.Filter.date(dates[timescale[i] - 2], ee.Date(str(tomorrow_date))))
               .mean())

    # define julian day start and end for
    julian_end = (ee.Date(dates[0]).getRelative('day', 'year').getInfo())
    julian_start = (ee.Date(dates[timescale[i] - 2]).getRelative('day', 'year').getInfo())
    ...
    z_score = ((current.subtract(julian_clip.mean())) \
               ...
               .set('time_start', str(dates[timescale[i] - 2]))
