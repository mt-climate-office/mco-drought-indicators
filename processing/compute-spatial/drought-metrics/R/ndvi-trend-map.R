...
outline = rgdal::readOGR("/home/zhoylman/drought_indicators/shp_kml/larger_extent/outline_umrb.shp")
...
urls = read_csv('/home/zhoylman/drought_indicators/ndvi/data/urls/url_list_greeness_trend.csv', col_names = F) %>%
  t()
...
for(i in 1:length(timescales)){
  ...
  writeRaster(data, paste0('/home/zhoylman/drought_indicators/ndvi/data/maps/ndvi_trend_', timescales[i], '.tif'), overwrite = T)
  ...
  layer_name = paste("ndvi_trend_watershed_",as.character(timescales[i]), sep = "")
  ...
  layer_name = paste("ndvi_trend_county_",as.character(timescales[i]), sep = "")
  ...
  path_file_tribe = paste("/home/zhoylman/drought_indicators/ndvi/data/shp/current_ndvi/ndvi_trend_tribal_",as.character(timescales[i]),'.shp', sep = "")
